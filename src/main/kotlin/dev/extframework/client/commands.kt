...
internal class ProductionCommand(
    ...
    val mainClass by option().required()
    val classpath by option().required()
    val gameJar by option().required()
...
//        echo("Launch context built: $launchContext")
    }
...
//
////        RequiredGameInfo(
////            ClasspathApp(classpath.split(CP_SPLIT_PATH).map(::Path), version, gameDir),
