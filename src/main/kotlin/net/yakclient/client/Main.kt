...
public fun main(args: Array<String>) {
    ...
    val factory = boot.componentGraph.get(request.descriptor).orNull()?.factory!! as ComponentFactory<ComponentConfiguration, ComponentInstance<ComponentConfiguration>>

    val extensions = if (System.`in`.available() == 0) listOf() else ObjectMapper().registerModule(KotlinModule.Builder().build()).readValue<List<Map<String, Any>>>(System.`in`)
    val config = factory.parseConfiguration(ContextNodeTypes.newValueType(mapOf(
